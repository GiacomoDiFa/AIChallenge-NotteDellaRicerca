...
class SharedData:
    ...
    def __init__(self):
        ...
        # Semaforo per il reset per implementare pattern produttore-consumatore settato a 0 permessi
        self.reset_semaphore = threading.Semaphore(0)
...
def reset_variables(shared_data):
    ...
    # Scelgo un testo e la rispettiva voce in maniera casuale
    shared_data.randomText = random.choice(list(texts['texts']))
    shared_data.randomVoice = pygame.mixer.Sound(shared_data.randomText['voice'])
...
def main_thread(shared_data):
    ...
    while True:
        ...
        result = model(frame, classes=[0], conf=0.75)
        ...
        with shared_data.detection_lock:
            ...
            if shared_data.application_detections == 0 or shared_data.detection_number > MAX_DETECTION_NUMBER:
                ...
                if shared_data.perso == 1:
                    ...
                    sound2.play()
                    # Eseguo la voce
                    shared_data.randomVoice.play()
                    # Rilascio il semaforo contatore
                    shared_data.reset_semaphore.release()
        # Rilascio il lock
...
def reset_game_thread(shared_data):
    def getUptime():
    ...
    while True:
        # Acquisisco semaforo contatore
        shared_data.reset_semaphore.acquire()
        # Mi devo resettare dopo un po' che ho perso
        # Acquisisco il lock
        wait(10)
        reset_variables(shared_data)
        # Rilascio il lock
...
reset_thread_instance = threading.Thread(target=reset_game_thread, args=(shared_data,))

# Esecuzione dei thread
main_thread_instance.start()
dc_thread_instance.start()
reset_thread_instance.start()

# Attesa della terminazione dei thread
main_thread_instance.join()
dc_thread_instance.join()
reset_thread_instance.join()