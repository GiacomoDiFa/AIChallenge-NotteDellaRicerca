...
def detection_tracker():
    global detection_number, application_detections,iou, soglia_IOU
...
def calculate_area(x0,y0,x1,y1):
    base = np.sqrt(((x0-x1)**2) + ((y0-y0)**2))
    altezza = np.sqrt(((x0-x0)**2) + ((y0-y1)**2))
    area = base * altezza
    return area
...
while True:
    ...
    if len(array) > 0:
        ...
        #    detection_list.append((int(array[0][0]),int(array[0][1]),int(array[0][2]),int(array[0][3])))
        #    detection_list.append((int(array[0][0]),int(array[0][1]),int(array[0][2]),int(array[0][3])))
        #else:
        #    detection_list.append((int(array[0][0]),int(array[0][1]),int(array[0][2]),int(array[0][3])))
        #    detection_list.pop(0)
        # with soglie_lock:
        #     iou = calculate_IOU(detection_list[0],detection_list[1])
        #     print(iou)
